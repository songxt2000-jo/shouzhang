...
我们自己是让家里的Claude Code照着它部署的，所以下文的命令都写得能直接执行。如果你家也有AI搭档，把这个仓库丢给TA就行；自己动手当然也没问题。需要人来做的只有几件事：准备一台VPS（或一台常开的电脑）、在云控制台放行443端口、想一个口令。
...
**⚠️ 一个我们踩过的坑**：如果这台VPS上已经跑着别的重要服务（比如一个24小时值班的AI），建议把本应用做成独立进程并加资源上限，这样它就算出错也只影响自己。下面的systemd配置已经带了这层保护，是我们吃过一次苦头之后加的。
...
这东西当手帐用比当监工用要长久。印章断了不催，照片糊了也夸，页边的字写给对方看，不写给绩效看。仅供参考，你们家的用法你们自己长出来。
